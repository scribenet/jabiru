...
class CompileCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ->setName('phar')
            ->setDescription('Compiles Jabiru into a phar archive')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $script = file_get_contents('bin/jabiru');
        $script = preg_replace('/^.*?(<\?php.*)/ms', '\1', $script);
        $phar->addFromString('bin/jabiru', $script);
    ...
    {
        return "#!/usr/bin/env php\n<?php Phar::mapPhar('jabiru.phar'); require 'phar://jabiru.phar/bin/jabiru'; __HALT_COMPILER();";
    }
